...
//			int y = Integer.parseInt(mr.getParameter("y"));
	...
	public static void login(HttpServletRequest req, HttpServletResponse res) {
		Connection con = null;
		// 연결 준비 -				🔹 DB와의 연결을 준비하는 객체 (아직 연결되지 않았으며, 이후 실제 연결을 수행할 예정)
		PreparedStatement pstmt = null;
		// 실행 준비 -				🔹 Connection을 통해 생성된 SQL 실행 준비 객체(PreparedStatement)
		//						🔹 SQL을 미리 준비(Precompile)하고, 실행할 수 있도록 설정함
		//						🔹  `?`(바인딩 변수)를 사용하여 동적 SQL 실행이 가능함
		ResultSet rs = null;
		// sql을 조회한 결과값 저장 - 🔹 SQL `SELECT` 조회 결과를 저장하는 객체
		//						🔹 아직 데이터가 없는 상태이며, `pstmt.executeQuery();`가 실행되면 DB에서 조회된 결과를 저장함
		//						🔹 사용자는 `rs.next()`를 사용하여 조회된 데이터를 읽을 수 있음
		try {
			con = BeaverDBManager.connect("sexy");
			// 🔹 '데이터베이스와의 연결'을 생성하는 역할 - BeaverDBManager 클래스의 connect() 메서드를 호출하여
			// 🔹 "sexy"라는 이름의 특정 데이터베이스(DB)와 연결을 생성하는 역할 con을 활용하여 SQL을 실행할 준비 상태
			req.setCharacterEncoding("UTF-8");
			// 🔹 dopost에서부터 요청하는 request, 요청받은 파라미터에 한글이 있다면 깨짐방지를 하기 위해 작성  
			// 🔹 doget에서는 자동으로 인코딩을 하기 때문에 해당 메소드를 활용하는 특정 컨트롤러(서블릿)의 dopost()에 요청파라미터가 있다는걸 유추할 수 있음
			String id = req.getParameter("id");
			// 🔹 특정 컨트롤러(서블릿)에서 전달된 요청 파라미터 `"id"` 값을 가져와 `String id`에 저장
			// 🔹 `req.getParameter("id")`의 반환값은 항상 문자열(`String`)이므로, `String id`에 저장 가능
			// 🔹 즉, 클라이언트가 입력한 `"id"` 값이 문자열로 넘어오며, 이를 해당 메소드에서 활용할 수 있도록 함
